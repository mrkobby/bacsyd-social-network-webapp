...
if (isset($_GET['action']) && $_GET['action'] == "status_post"){
	...
	$psql = "SELECT COUNT(id) FROM story WHERE author='$log_username' AND type='a'";
    $pquery = mysqli_query($db_connection, $psql); 
	$prow = mysqli_fetch_row($pquery);
	if ($prow[0] > 29) { 
		$osql = "SELECT id FROM story WHERE author='$log_username' AND type='a' ORDER BY id ASC";
    	$oquery = mysqli_query($db_connection, $osql); 
		$orow = mysqli_fetch_row($oquery);
		$oldest = $orow[0];
	...
	if($account_name != $log_username){
		mysqli_query($db_connection, "INSERT INTO alerts(story_id, username, initiator, app, note, date_time) VALUES('$id','$account_name','$log_username','$app','$note',now())");
	}
}
