<?php
$sql = "SELECT COUNT(id) FROM followers WHERE user1='$log_username' OR user2='$log_username'";
$query = mysqli_query($db_connection, $sql);
$query_count = mysqli_fetch_row($query);
$follower_count = $query_count[0];

	$all_followers = array();
	$wsql = "SELECT user2 FROM followers WHERE user1='$log_username'";
	$wquery = mysqli_query($db_connection, $wsql);
	while ($row = mysqli_fetch_array($wquery, MYSQLI_ASSOC)) {
		array_push($all_followers, $row["user2"]);
	}
	$followerArrayCount = count($all_followers);
	$orLogic = '';
	foreach($all_followers as $key => $follower){
			$orLogic .= "type='a' AND author='$follower' OR type='x' AND author='$follower' OR type='p' AND author='$follower' OR ";
	}
	$orLogic = chop($orLogic, "OR ");
		$followerquery = "$orLogic OR";
		if($orLogic == NULL){
			$followerquery = '';
		}
?><?php	
...
$avatar = "";
$datafont = "";
$sql = "SELECT * FROM story WHERE $followerquery type='a' AND author='$log_username' OR type='x' AND account_name='$log_username' OR type='x' AND author='$log_username' OR type='p' AND author='$log_username' ORDER BY postdate DESC LIMIT {$limit} OFFSET {$offset}";
$query = mysqli_query($db_connection, $sql);
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	while ($row = mysqli_fetch_array($_query, MYSQLI_ASSOC)) {
		...
		if($type == 'a'){
            ...
            $statuslist .= '<li><a href="javascript:void(0)" onclick="OpenShareCaption(\''.$statusid.'\',\''.$author.'\',\''.$data.'\',\''.$postdate.'\',\'sh_'.$statusid.'\',\''.$postpic.'\');">Add caption and share</a></li>';
