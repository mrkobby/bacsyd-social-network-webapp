...
$avatar = "";
$datafont = "";
$sql = "SELECT * FROM story WHERE author != '$log_username' AND type='a' OR type='x' ORDER BY postdate ASC LIMIT {$limit} OFFSET {$offset}";
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	while ($row = mysqli_fetch_array($_query, MYSQLI_ASSOC)) {
		...
		if($type == 'a'){
            ...
            $statuslist .= '<li id="sh_'.$statusid.'"><a href="javascript:void(0)" id="shareBtn'.$statusid.'" onclick="sharePost(\''.$statusid.'\',\''.$author.'\',\''.$data.'\',\''.$postdate.'\',\'sh_'.$statusid.'\',\''.$postpic.'\',\'\')">Share post</a></li>';
            $statuslist .= '</ul></li>'; */
			...
			$statuslist .= '<li class="pull-right"><span class="hand link-black text-sm" href="javascript:void(0)"><i class="fa fa-comments-o margin-r-5"></i> Comments ( <span id="rp_'.$statusid.'">'.$replynumrows.'</span> )</span></li>';
		    $statuslist .= '<li class="pull-right border-right"><span href="" class="link-black text-sm"><i class="fa fa-thumbs-o-up margin-r-5"></i><span id="lknum_'.$statusid.'"> '.$likenumrows.'</span></span></ul></div></div>';
